...
"""Sets up a Raspberry Pi for use as a TUM quadruped control computer.

The base operating system should already be installed.
Supported OS:
- Raspberry Pi OS Bookworm OR AKA Raspberry Pi OS 32-bit Legacy
...
wpa=2
wpa_passphrase= TalentumTum1234
...
''')
    ...
    ensure_contents('/etc/systemd/system/hostapd.service.d/10-select-ssid.conf',
                    '''
[Service]
ExecStartPre=bash -c "SSID=tum-$(iw dev wlan0 info | grep addr | perl -pe 's/.*addr //; s/://g'); perl -pi -e \"s/ssid=.*/ssid=$SSID/\" /etc/hostapd/hostapd.conf"
''')
...
''')
    ...
    ensure_contents('/etc/systemd/system/tum-quadruped.service', '''
[Unit]
Description=TUM Quadruped System
After=network.target

[Service]
Type=forking
ExecStart=/home/pi/tum-start.sh
...
''')
    ...
    subprocess.check_call("systemctl enable tum-quadruped.service", shell=True)
